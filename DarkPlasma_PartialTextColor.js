// DarkPlasma_PartialTextColor 1.0.0
 ...
 * @help
 * version: 1.0.0
 * drawTextによるテキスト描画でも部分文字列に色をつけられるようにします。
 * \C制御文字による色付けが可能になります。
 */
...
(() => {
  ...
  function Bitmap_PartialTextColorMixIn(bitmap) {
    ...
    bitmap._parseTextForPartialColor = function (text) {
      ...
      while ((match = regex.exec(text)) !== null) {
        ...
        const value = match[1];
        currentColor = /^\d+$/.test(value) ? ColorManager.textColor(parseInt(value, 10)) : value;
  ...
  function Window_Base_PartialTextColorMixIn(windowBase) {
