// DarkPlasma_ItemCommandCooldown 1.0.2
// Copyright (c) 2026 DarkPlasma
// This software is released under the MIT license.
// http://opensource.org/licenses/mit-license.php

/**
 * 2026/05/01 1.0.2 TypeScript移行
 *                  色設定をカラーパレット対応
...
(() => {
  ...
  const settings = {
    ...
    display: pluginParameters.display
      ? ((parameter) => {
          const parsed = JSON.parse(parameter);
          return {
            enabled: String(parsed.enabled || true) === 'true',
            format: String(parsed.format || `CT:{turn}`),
            color: parsed.color?.startsWith('#') ? String(parsed.color) : Number(parsed.color || 2),
          };
        })(pluginParameters.display)
      : { enabled: true, format: 'CT:{turn}', color: 2 },
  };

  function ColorManager_ItemCommandCooldownMixIn(colorManager) {
