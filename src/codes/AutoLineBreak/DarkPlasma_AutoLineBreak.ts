...
function Window_AutoLineBreakMixIn(windowClass: Window_Base) {
   ...
   * 自動折返しが有効かどうか
   */
  windowClass.isAutoLineBreakEnabled = function (force) {
  ...
  windowClass.processCharacter = function (textState) {
    if (textState.drawing) {
      textState.targetLineWidth = this.lineWidth();
    }
    if (this.shouldLineBreakHere(textState, { lineBuffer: textState.lineBuffer , index: textState.index })) {
   ...
   * 自動改行すべき状態であるかどうか
   */
  windowClass.shouldLineBreakHere = function (textState, lineBreakState) {
    if (!textState || lineBreakState.index === 0 || !textState.text[lineBreakState.index] || !this.isAutoLineBreakEnabled(lineBreakState.forceLineBreak)) {
      return false;
    }
    return this.shouldCharacterBaseLineBreakHere(textState, lineBreakState) || this.shouldWordBaseLineBreakHere(textState, lineBreakState);
  };

  windowClass.shouldCharacterBaseLineBreakHere = function (textState, lineBreakState) {
    let nextCharacter = textState.text[lineBreakState.index];
    if (this.isSurrogatePair(nextCharacter)) {
      return false;
    }
    let next2Character = this.nextNCharacter(textState, lineBreakState, 2);
    let next3Character = this.nextNCharacter(textState, lineBreakState, 3);
    const size = this.textWidth(`${lineBreakState.lineBuffer}${nextCharacter}`);
    if (size + textState.x > this.lineWidth(textState.targetLineWidth)) {
      return !this.isProhibitLineBreakBefore(nextCharacter);
    } else if (
      next2Character &&
      next3Character &&
      size + textState.x + this.textWidth(`${next2Character}${next3Character}`) > this.lineWidth(textState.targetLineWidth)
    ) {
    ...
    if (
      next2Character &&
      this.textWidth(`${lineBreakState.lineBuffer}${nextCharacter}${next2Character}`) + textState.x > this.lineWidth(textState.targetLineWidth)
    ) {
  ...
  windowClass.shouldWordBaseLineBreakHere = function (textState, lineBreakState) {
    if (!settings.wordBaseLineBreak) {
      return false;
    }
    const isInitialOfWord = textState.text[lineBreakState.index-1] === " " && textState.text[lineBreakState.index] !== " ";
    const nextSpaceIndex = Math.min(
      textState.text.indexOf(" ", lineBreakState.index+1),
      textState.text.indexOf("\n", lineBreakState.index+1)
    );
    if (!isInitialOfWord || nextSpaceIndex < 0) {
      return false;
    }
    const currentWord = textState.text.substring(lineBreakState.index, nextSpaceIndex);
    const size = this.textWidth(`${lineBreakState.lineBuffer}${currentWord}`);
    return size + textState.x > this.lineWidth(textState.targetLineWidth);
  };

  /**
   * N文字先の文字
   */
  windowClass.nextNCharacter = function (textState, lineBreakState, n) {
    let targetIndex = lineBreakState.index + n - 1;
   ...
   * 折返し幅
   */
  windowClass.lineWidth = function (width?: number) {
    return width || (this.contentsWidth() - settings.lineWidthMargin);
  };
...
function Window_MessageDummy_MessageAlignCenterMixIn(windowClass: Window_MessageDummy) {
  windowClass.findLineText = function (textState) {
    let text = '';
    let index = textState.index;
    while(textState.text[index] !== '\n' && !!textState.text[index] && !this.shouldLineBreakHere(textState, {lineBuffer: text, index, forceLineBreak: true})) {
...
function Window_Base_MessageAlignCenterMixIn(windowBase: typeof Window_Base) {
  const _createDummyWindow = windowBase.createDummyWindow;
  windowBase.createDummyWindow = function () {
    const result = _createDummyWindow.call(this);
    Window_MessageDummy_MessageAlignCenterMixIn(result);
    return result;
  };
}

Window_Base_MessageAlignCenterMixIn(Window_Base);
